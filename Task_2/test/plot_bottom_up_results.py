...
for i in range(len(algos)):
    with open('../data/results/' + algos[i].__name__ + '_bottom_up.npy','rb') as f:
        algo_data[i] = np.load(f)


best_fit_bottom_up = np.mean(algo_data[0],axis=0)

first_fit_bottom_up = np.mean(algo_data[1],axis=0)

best_fit_post_order_bottom_up = np.mean(algo_data[2],axis=0)

best_fit_level_order_bottom_up = np.mean(algo_data[3],axis=0)
...
fig, ax = plt.subplots()
"""
rects1 = ax.bar(x - width, best_fit_bottom_up,width/2,label='Algo_1-best-fit')
rects2 = ax.bar(x - width/2, first_fit_bottom_up,width/2,label='Algo_1-first-fit')
rects3 = ax.bar(x , best_fit_post_order_bottom_up,width/2,label='Algo_2')
rects4 = ax.bar(x + width/2, best_fit_level_order_bottom_up,width/2,label='Algo_3')
"""
ax2 = ax.twinx()
...
ax.set_title('Leaf to root traversal time')
ax.set_xticks(x)
